...
const sanitizedNameSchema = zod
  .string()
  .transform((value) => sanitizeText(value, { maxLength: 80 }))
  .pipe(zod.string().min(2, "Name must be at least 2 characters").max(80, "Name is too long"));

const sanitizedEmailSchema = zod
  .string()
  .trim()
  .toLowerCase()
  .email();

const sanitizedBioSchema = zod
  .string()
  .transform((value) => sanitizeText(value, { maxLength: 500, allowNewlines: true }))
  .pipe(zod.string().max(500, "Bio is too long"));
...
export const RegisterBody = zod.object({
  ...
  password: zod.string().min(registerBodyPasswordMin).max(128),
}).strict();
