...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background overflow-hidden">
      ...
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-[60px] flex-shrink-0 flex-col items-center bg-sidebar border-r border-sidebar-border py-4 z-[200] isolate pointer-events-auto shadow-xl">
        {/* Logo mark */}
        ...
        {/* Nav icons */}
        <nav className="flex flex-col items-center gap-1 flex-1 w-full overflow-y-auto overflow-x-hidden pb-2">
          {visibleItems.map((item) => {
            const isActive = pathname === item.href || pathname.startsWith(item.href + "/");
            return (
              <div
                key={item.href}
                className="relative w-full flex items-center justify-center py-1 flex-shrink-0"
        ...
        <div className="mt-auto flex flex-col items-center gap-3 w-full flex-shrink-0 pt-2">
          <div
      ...
      {/* Main content area */}
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto relative z-0 md:ml-[60px] md:w-[calc(100%-60px)] md:max-w-[calc(100%-60px)]">
