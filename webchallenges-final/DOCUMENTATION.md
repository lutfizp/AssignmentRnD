# Final Documentation: Remediated Build

## Overview

`webchallenges-final` is the final patched build of the application. It starts from the hardened Part 2 implementation and adds one more remediation step aimed at removing the `React2Shell` execution path from the runtime.

The goal of this folder is different from Part 1 and Part 2:

- Part 1 demonstrates the functional application
- Part 2 demonstrates security hardening and code-level improvements
- Final demonstrates the remediated build that is intended to stay operational while closing the specific `React2Shell` attack surface

## What Was Patched in Final

The final patch is intentionally narrow and only applies to this folder.

### 1. React Server Actions Surface Removed from Configuration

In earlier builds, [`next.config.mjs`](https://github.com/lutfizp/AssignmentRnD/blob/master/webchallenges-final/next.config.mjs) enabled:
...
The final build removes that configuration because this application does not use Server Actions as part of its feature set.

### 2. `Next-Action` Requests Are Explicitly Blocked

The final build adds request filtering in [`middleware.ts`](https://github.com/lutfizp/AssignmentRnD/blob/master/webchallenges-final/middleware.ts).

If a request is:

- `POST`
- and carries the `Next-Action` header

the middleware returns `403` immediately instead of letting the request reach the application runtime.

This matters because the exploit tooling in the repository targets the `Next-Action` request path directly. Blocking it in middleware closes that entry point before route logic or rendering code is reached.

### 3. Framework Version Updated to a Patched Release

The final build also updates [`package.json`](https://github.com/lutfizp/AssignmentRnD/blob/master/webchallenges-final/package.json) from `next@15.1.7` to `next@15.1.11`, which is the patched release line for this application's Next.js version family.

That change is important because the middleware and config hardening in this repository are best treated as defense-in-depth. The primary remediation should still include the official framework patch level.

### 4. Scope of Change

Only `webchallenges-final` is patched this way.

- `webchallenges-part1` remains the original functional baseline
- `webchallenges-part2` remains the security-hardening stage
- `webchallenges-final` is the remediated release candidate

That separation keeps the repository easy to review because the evaluator can still see the progression from implementation, to hardening, to final remediation.

## Why This Mitigates React2Shell

The React2Shell tooling present in [`exploit/`](https://github.com/lutfizp/AssignmentRnD/tree/master/exploit) attempts to send crafted requests that rely on the `Next-Action` mechanism to reach server-side execution behavior.

The final build reduces that risk in two complementary ways:

- it upgrades Next.js to the patched `15.1.11` release
- it no longer enables the experimental Server Actions configuration
- it rejects `Next-Action` requests at the middleware layer

Because the application itself does not require Server Actions, this patch does not remove needed business functionality.

## Related Files

- [`webchallenges-final/next.config.mjs`](https://github.com/lutfizp/AssignmentRnD/blob/master/webchallenges-final/next.config.mjs)
- [`webchallenges-final/middleware.ts`](https://github.com/lutfizp/AssignmentRnD/blob/master/webchallenges-final/middleware.ts)
- [`webchallenges-final/README.md`](https://github.com/lutfizp/AssignmentRnD/blob/master/webchallenges-final/README.md)
- [`wiki/Final.md`](https://github.com/lutfizp/AssignmentRnD/blob/master/wiki/Final.md)

## Final Note

This documentation page describes the remediation status of the final build. The future pentest report can later be linked from the wiki placeholder once the external assessment document is ready.
